...
def dipole_dipole(x1, x2, a):
    ...
    A, B, M, N, X, Y = [], [], [], [], [], []
    max_n = len(elektroda)  # estimasi max level n
    ...
    return np.array(A), np.array(B), np.array(M), np.array(N), np.array(X), np.array(Y), elektroda
...
def run(x1, x2, a, output_dir=".", plot=True):
    ...
    if plot:
        ...
        # Judul dan label
        ax.set_title('Stacking Chart of Dipole-Dipole Configuration', fontsize=14, pad=20)
